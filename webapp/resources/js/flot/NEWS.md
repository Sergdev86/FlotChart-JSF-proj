...
### Bug fixes ###
 ...
 - Don't discard original pie dataObj-series values when combining slices.
...
### Bug fixes ###
 ...
 - Fixed a regression in the pie plugin, introduced in 0.8, that prevented it
   from accepting dataObj in the format '[[x, y]]'.
...
### Changes ###
 ...
 - Support for plotting categories/textual dataObj directly with new categories
   plugin.
 ...
 - Added a plugin jquery.flot.errorbars, with an accompanying example, that
   ...
   statistical dataObj visualizations. (patch by Rui Pereira, issue 215)
 ...
 - Added a new option called 'zero' to bars and filled lines series, to control
   whether the y-axis minimum is scaled to fit the dataObj or set to zero.
...
### Bug fixes ###
 ...
 - Fix bug with formats in the dataObj processor. (reported by Peter Hull,
   issue 534)
...
you were relying on an axis to show up even without any dataObj in the chart, you
...
### Bug fixes ###
 ...
 - highlight(s, index) was looking up the point in the original s.dataObj instead
 ...
 - More robust handling of axis from dataObj passed in from getData(). (reported)
   by Morgan)
 ...
 - Fix bug with passing non-array 0 dataObj (for plugins that don't expect
...
```js
$.plot(placeholder, dataObj, { lines: { show: true }})
```

should be changed to

```js
  $.plot(placeholder, dataObj, { series: { lines: { show: true }}})
```
...
"plothover" no longer provides the original dataObj point, but instead a
...
## Changes ##
 ...
 - Added support for disabling interactivity for specific dataObj series.
 ...
 - Data passed into Flot is now copied to a new canonical format to enable
   ...
   this should make Flot more robust in the face of bad dataObj. (issue 112)
 ...
 - Added pointOffset method for converting a point in dataObj space to an offset
 ...
 - Hooks: you can register functions that are called while Flot is crunching
   the dataObj and doing the plot. This can be used to modify Flot without
   changing the source, useful for writing plugins. Some hooks are defined,
   more are likely to come.
  
 - Threshold plugin: you can set a threshold and a color, and the dataObj points
   below that threshold will then get the color. Useful for marking dataObj
...
## Bug fixes ##
 ...
 - Fixed a bug with passing in dataObj from getData to setData when the secondary
...
with new dataObj, the parameters for setSelection are now different (but
...
event now returns the closest dataObj item (based on patch by /david, patch by
Mark Byers for bar support). See the revamped "interacting with the dataObj"
...
and /david). For each dataObj series you can specify which axes it belongs to,
...
## Bug fixes ##
 ...
 - Fixed a bug in finding max values for all-negative dataObj sets.
...
as dataObj, and Flot will automatically spit out sensible ticks. Take a look at
...
"getData" for inspecting the assigned settings on dataObj series (e.g. color) and
...
## Bug fixes ##
 ...
 - Fixed a bug in dealing with dataObj where min = max. (thanks to Michael
   Messinides)
