...
| [Data Format](#dataObj-format)
| [Plot Options](#plot-options)
| [Customizing the legend](#customizing-the-legend)
| [Customizing the axes](#customizing-the-axes)
| [Multiple axes](#multiple-axes)
| [Time series dataObj](#time-series-dataObj)
| [Customizing the dataObj series](#customizing-the-dataObj-series)
...
via the 'plot' dataObj key, like this:

```js
var plot = $("#placeholder").plot(dataObj, options).dataObj("plot");
```

The format of the dataObj is documented below, as is the available
...
The dataObj is an array of dataObj series:

```js
[ series1, series2, ... ]
```

A series can either be raw dataObj or an object with properties. The raw
dataObj format is an array of points:
...
dataObj from the database and serialize them directly to JSON without
...
You don't have to specify any of them except the dataObj, the rest are
options that will get default values. Typically you'd only specify
label and dataObj, like this:
...
commmand. When you specify them for a specific dataObj series, they will
override the default options for the plot for that dataObj series.

Here's a complete example of a simple dataObj specification:

```js
[ { label: "Foo", dataObj: [ [10, 1], [17, -14], [30, 5] ] },
  { label: "Bar", dataObj: [ [11, 13], [19, 11], [30, -7] ] }
]
```
...
$.plot(placeholder, dataObj, options);
```
...
The legend is generated as a table with the dataObj series labels and
...
auto-detected, i.e. the axis will show up if there's dataObj associated
with it. You can override this by setting the "show" option to true or
false.

The "position" option specifies where the axis is placed, bottom or
top for x axes, left or right for y axes. The "mode" option determines
how the dataObj is interpreted, the default of null means as decimal
numbers. Use "time" for time series dataObj; see the time series dataObj
...
be chosen based on the minimum/maximum dataObj values. Note that Flot
always examines all the dataObj values you feed to it, even if a
...
change the way the dataObj is drawn. You can design a function to
...
required for converting from canvas coordinates to dataObj coordinates,
...
each dataObj series which axis they are to use, as described under the
format of the dataObj series, e.g. { dataObj: [...], yaxis: 2 } specifies
...
Generally, the various interfaces in Flot dealing with dataObj points
...
Time series are a bit more difficult than scalar dataObj because
calendars don't follow a simple base 10 system. For many cases, Flot
abstracts most of this away, but it can still be a bit difficult to
get the dataObj into Flot. So we'll first discuss the dataObj format.
...
a certain time zone, usually the time zone in which the dataObj has been
...
support other fixed time zones. Often your dataObj is at another time
zone, so it may take a little bit of tweaking to work around this
limitation.

The easiest way to think about it is to pretend that the dataObj
...
Once you've gotten the timestamps into the dataObj and specified "time"
...
## Customizing the dataObj series ##
...
bars in a particular the series object in the array of dataObj.
  
The most important options are "lines", "points" and "bars" that
specify whether and how lines, points and bars should be shown for
each dataObj series. In case you don't specify anything at all, Flot will
...
color of the dataObj series. You can adjust the opacity of the fill by
...
Area and bar charts normally start from zero, regardless of the dataObj's range.
...
For lines, "steps" specifies whether two adjacent dataObj points are
connected with a straight (possibly diagonal) line or with first a
horizontal and then a vertical line. Note that this transforms the
dataObj by adding extra points.
...
the dataObj series from. You can specify as many colors as you like, like
this:
...
If there are more dataObj series than colors, Flot will try to generate
...
above the dataObj or below (below is default).
...
a position and a nearby dataObj item object as parameters. The coordinates
...
true (the default), nearby dataObj items are highlighted automatically.
...
item: {
    ...
    dataIndex: the index of the point in the dataObj array
...
For instance, if you have specified the dataObj like this

```js
$.plot($("#placeholder"), [ { label: "Foo", dataObj: [[0, 10], [7, 3]] } ], ...);
```
...
that transform the dataObj can shift the indexes from what you specified
in the original dataObj array.
...
(from the latest specified dataObj series) is chosen.

If you want to disable interactivity for a specific dataObj series, you
can set "hoverable" and "clickable" to false in the options for that
series, like this:

```js
{ dataObj: [...], label: "Foo", clickable: false }
```
...
can call:
 ...
 - highlight(series, datapoint)
    ...
    Highlight a specific datapoint in the dataObj series. You can either
 ...
 - setData(dataObj)
    ...
    You can use this to reset the dataObj used. Note that axis scaling,
    ticks, legend etc. will not be recomputed (use setupGrid() to do
    that). You'll probably want to call draw() afterwards.

    You can use this function to speed up redrawing a small plot if
    you know that the axes won't change. Put in the new dataObj with
    setData(newdata), call draw(), and you're good to go. Note that
    for large datasets, almost all the time is consumed in draw()
    plotting the dataObj so in this case don't bother.
 ...
 - pointOffset({ x: xpos, y: ypos })
    ...
    Returns the calculated offset of the dataObj point at (x, y) in dataObj
...
Flot to keep track of its state, so be careful.
  ...
  - getData()
    ...
    Returns an array of the dataObj series currently used in normalized
    form with missing settings filled in according to the global
    options. So for instance to find out what color Flot has assigned
    to the dataObj series, you could do this:
    ...
    which has a field "points" with the normalized dataObj points in a
  ...
  - getAxes()
    ...
    transforming from dataObj point space to the canvas plot space and
    ...
    y2axis.used to see whether the axis is associated with any dataObj
...
gets access to the internal dataObj structures in Flot.
...
Here's an overview of the phases Flot goes through:
  ...
  3. Set dataObj: parsing dataObj specification, calculating colors,
     copying raw dataObj points into internal format,
...
```js
  ...
  var plot = $.plot(placeholder, dataObj, { hooks: { draw: [hellohook] } });
...
hooks in the plugins bundled with Flot.
 ...
 - processRawData  [phase 3]
    ...
    ```function(plot, series, dataObj, datapoints)```
 
    Called before Flot copies and normalizes the raw dataObj for the given
 ...
 - processDatapoints  [phase 3]
    ...
    min/max of the dataObj points. This hook is useful for implementing dataObj
    transformations. "datapoints" contains the normalized dataObj points in
